...
class MainWindow(QWidget):
    ...
    def setup_timers(self):
        ...
        self.sync_timer.timeout.connect(self.periodic_time_sync_attempt)
        self.sync_timer.start(self.sync_interval * 1000)

        # Timer for periodic data syncing
        self.sync_timer = QTimer(self)
        self.sync_timer.timeout.connect(self.periodic_data_sync_attempt)
        self.sync_timer.start(30 * 1000)  # 30 seconds in milliseconds
    ...
    def periodic_data_sync_attempt(self):
    ...
    def sync_data(self):
        ...
        if is_internet_available():
            if sync_staff_data() and sync_schedule_data() and sync_temp_schedule_data():
                self.populate_table()
                print(f"data synchronized successfully at App time: {self.current_datetime.strftime('%Y-%m-%d %H:%M:%S')}, System time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
            else:
                print(f"Failed to synchronize data. Using local data. {self.current_datetime.strftime('%Y-%m-%d %H:%M:%S')}")
        else:
            print(f"No internet connection. Using local data. {self.current_datetime.strftime('%Y-%m-%d %H:%M:%S')}")
